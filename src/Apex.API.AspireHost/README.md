...
1. Set `Apex.API.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../Apex.API.Infrastructure/Apex.API.Infrastructure.csproj -s Apex.API.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../Apex.API.Infrastructure/Apex.API.Infrastructure.csproj -s Apex.API.Web.csproj
```
