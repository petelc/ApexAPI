...
/// <summary>
/// Handler for approving a ProjectRequest
/// </summary>
public class ApproveProjectRequestHandler : IRequestHandler<ApproveProjectRequestCommand, Result>
{
    private readonly IRepository<ProjectRequest> _repository;
    private readonly ITenantContext _tenantContext;
    private readonly ICurrentUserService _currentUserService;
    private readonly ILogger<ApproveProjectRequestHandler> _logger;

    public ApproveProjectRequestHandler(
        IRepository<ProjectRequest> repository,
        ITenantContext tenantContext,
        ICurrentUserService currentUserService,
        ILogger<ApproveProjectRequestHandler> logger)
    {
    ...
    public async Task<Result> Handle(
        ApproveProjectRequestCommand command,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Approve the ProjectRequest (business logic in aggregate)
            projectRequest.Approve(_currentUserService.UserId, command.Notes);

            await _repository.UpdateAsync(projectRequest, cancellationToken);

            _logger.LogInformation(
                "ProjectRequest approved: ProjectRequestId={ProjectRequestId}, ApprovedBy={UserId}",
                command.ProjectRequestId,
                _currentUserService.UserId);
        ...
        {
            _logger.LogError(ex, "Error approving ProjectRequest: ProjectRequestId={ProjectRequestId}", command.ProjectRequestId);
            return Result.Error("An error occurred while approving the ProjectRequest.");
        }
    }
}
